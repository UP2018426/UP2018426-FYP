...
namespace UltimateXR.Manipulation
{
    ...
    public class UxrGrabbableObject : UxrComponent<UxrGrabbableObject>, IUxrGrabbable
    {
        ...
        public float constraintAngle1;
        public float constraintAngle2;
        ...
        private void ConstrainTransform(bool processPosition, bool processRotation, Vector3 unprocessedLocalPosition, Quaternion unprocessedLocalRotation)
        {
            ...
            if (processRotation && _rotationConstraintMode != UxrRotationConstraintMode.Free)
            {
                ...
                {
                    targetLocalRotation = ClampRotation(transform.localRotation, unprocessedLocalRotation, InitialLocalRotation, _rotationAngleLimitsMin, _rotationAngleLimitsMax, false, ref _singleRotationAngleCumulative); // ahhhhhh
                }
        ...
        private Quaternion ClampRotation(Quaternion rot, Quaternion rotBeforeUpdate, Quaternion initialRot, Vector3 eulerMin, Vector3 eulerMax, bool invertRotation, ref float singleRotationAngle)
        {
            ...
            Vector3 rotationAxis               = (UxrAxis)singleAxisIndex; //á
            Vector3 perpendicularAxis          = ((UxrAxis)singleAxisIndex).Perpendicular;
            Vector3 initialPerpendicularVector = initialRot * perpendicularAxis; //á
            Vector3 currentPerpendicularVector = Vector3.ProjectOnPlane(rot * perpendicularAxis, rotationAxis); //á
            float   angle                      = Vector3.SignedAngle(initialPerpendicularVector, currentPerpendicularVector, rotationAxis) * (invertRotation ? -1.0f : 1.0f); //á
            float   angleDelta                 = angle - singleRotationAngle.ToEuler180(); //á

            //THIS HERE

            constraintAngle1 = angle;
            constraintAngle2 = angleDelta;
        ...
        public float FindAngle(Quaternion rot, Quaternion rotBeforeUpdate, Quaternion initialRot, ref float singleRotationAngle)
        {
            //int rangeOfMotionAxisCount = RangeOfMotionRotationAxisCount;
            ...
            Vector3 initialPerpendicularVector = initialRot * perpendicularAxis; //á
            Vector3 currentPerpendicularVector = Vector3.ProjectOnPlane(rot * perpendicularAxis, rotationAxis); //á
            float angle = Vector3.SignedAngle(initialPerpendicularVector, currentPerpendicularVector, rotationAxis); //á
            float angleDelta = angle - singleRotationAngle.ToEuler180(); //á
        ...
        public float                               _singleRotationAngleCumulative;
